...
# TODO: Fine tune label format, data-label should be similar
...
# Parameters
size_batch = 20  # Batch size
valid_size = 0.1  # Validation percentage in training
size_out = 2  # Size of the output layer (If Softmax use 2)
size_inp = 16  # Size of the input layer
size_state = 50  # Size of the LSTM
num_layer = 2  # Number of LSTM layers (depth)

grad_clip = 5  # Gradient clipping
back_prop_length = 5  # BPTT rate (accumulate # samples then BP)
forget_bias = 1  # Bias of the forget gate / Required for initialization
max_num_iter = 250  # Maximum number of epochs
valid_epoch = 10  # Perform validation after # of epochs
...
for i in range(max_num_iter):
    ...
    for b_idx in range(int(len(x_train) / size_batch)):
        ...
        for k in range(0, num_samples, back_prop_length):
            # Get batches
            x = [Variable(x_b_train[:, k + n]) for
                 n in range(back_prop_length)]
            y = [Variable(y_b_train[:, k + n]) for n in range(
                back_prop_length)]
...
plt.grid(True)
plt.savefig('DSFigTrain.pdf', format='pdf')
...
for j, (x, y, p) in enumerate(zip(x_valid, y_valid, prob_hat.swapaxes(0, 1))):
    ...
    plt.savefig(str(count) + 'DSFig.pdf', format='pdf')
    count += 1
